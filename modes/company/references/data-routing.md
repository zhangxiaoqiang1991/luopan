...
1. `westock-data profile <code>` — 公司概述、行业分类、上市日期
2. `westock-data finance <code> --num 4` — 多期利润表/资产负债表/现金流量表（一条命令同时出三大表）
3. `westock-data consensus <code>` — 分析师一致预期（EPS/收入/净利润预测 + PE/PS/ROE 估值区间）
4. `westock-data kline <code> --period month --limit 12` — 1 年价格走势背景
...
- 行情数据优先使用 westock-data 获取：`kline` 提供历史行情，`consensus` 提供一致预期估值。港股/美股的数据延迟需在输出中标注。具体能力边界见 westock-data 的 routing-guide.md §六。
